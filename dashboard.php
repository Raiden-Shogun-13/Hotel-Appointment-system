...
<main class="page-container">
  ...
  <section id="booking" aria-labelledby="booking-title" style="margin-top:2rem;">
        ...
        <table role="grid" aria-describedby="appointments-info">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td data-label="Status"><span style="background: var(--brand-1); color:#fff; padding:0.3rem 0.6rem; border-radius:6px; font-size:0.85rem; font-weight:600;"><?= ucfirst($app['status']) ?></span></td>
              <td data-label="Action" class="action-link">
                <?php if (in_array($app['status'], ['pending', 'confirmed'])): ?>
                  <a href="reschedule_appointment.php?id=<?= $app['id'] ?>" aria-label="Reschedule appointment for <?= htmlspecialchars($app['service_name']) ?>">Reschedule</a>
                  &nbsp;|&nbsp;
                  <a href="cancel_appointment.php?id=<?= $app['id'] ?>"
                     onclick="return confirm('Are you sure you want to cancel this appointment?')"
                     aria-label="Cancel appointment for <?= htmlspecialchars($app['service_name']) ?>">Cancel</a>
                <?php else: ?>
                  &mdash;
                <?php endif; ?>
              </td>
